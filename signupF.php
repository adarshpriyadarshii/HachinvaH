...
if(isset($_POST['signup'])){
    ...
    if(empty($user) || empty($password)|| empty($cpassword)){
        header("Location: signupF.php?error=You Have one or more file empty");
        exit();
        ...
        if($result){
            header("Location: login.php?error=You Have Successfully signed in");
        exit();
        }else{
...
<body>
    <form class="box" action="#" method="POST">
        <h1>SIGN UP FAILED | HackinvaH</h1>
        <h1>TRY AGAIN !</h1>
